# <font size=15>Part 2: Three examples</font>
## <font color=#1E90FF size=5>例1：社会关系地位与幸福感的关系</font>
<font size=3>
实例的数据来自 Many Labs 2 项目中的一个研究。
...
&emsp;&emsp;&emsp;图中横坐标代表高低两种社会关系地位，纵坐标代表了主观幸福感评分。
...
- 这意味着**我们无法在传统显著性水平（α = 0.05）下拒绝原假设( 
H0
 )**。因此高社会地位和低社会地位下的幸福感差异在统计学上不是显著的。
...
&emsp;&emsp;1、通过建立线性模型去替代原本的t检验模型。
&emsp;&emsp;2、通过PyMC对后验进行采样
...
## <font color=#1E90FF size=5>例2：贝叶斯分层模型（Hierarchical/Multi-level Model） 可以更准确地估计信度</font>

&emsp;&emsp;在前面的内容中，我们通过社会关系地位（高与低）对幸福感的影响进行了可视化，并使用贝叶斯因子和贝叶斯推断对模型进行了进一步的分析。
...
## <font color=#1E90FF size=5>例3: 贝叶斯推断在认知模型中的应用</font>
